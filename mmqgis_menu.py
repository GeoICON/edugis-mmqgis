...
class mmqgis_menu:
	...
	def initGui(self):
		...
		self.grid_action = QAction(icon, "Create Grid Lines Layer", self.iface.mainWindow())
		QObject.connect(self.grid_action, SIGNAL("triggered()"), self.grid)
		self.create_menu.addAction(self.grid_action)

		icon = QIcon(os.path.dirname(__file__) + "/icons/mmqgis_points.png")
		self.points_action = QAction(icon, "Create Grid Points Layer", self.iface.mainWindow())
		QObject.connect(self.points_action, SIGNAL("triggered()"), self.points)
		self.create_menu.addAction(self.points_action)
	...
	def points(self):
		dialog = mmqgis_points_dialog(self.iface)
		dialog.exec_()
